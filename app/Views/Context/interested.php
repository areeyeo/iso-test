<title>Interested Party Version
  <?= $data['num_ver'] ?>
</title>
<!-- DataTables -->
...
<body class="hold-transition sidebar-mini">
                ...
                <div class="card-tools">
                  ...
                  <button class="badge badge-edit <?= $disabled ?>"
                    style="background-color: #FFFFFF;  border: 1px solid #ADB5BD;"><a
                      href="<?= site_url('context/interested_party/index/' . $data['type_version']); ?>"
                      style="color: #ADB5BD;">Version</a></button>
                  <button class="badge badge-edit <?= $disabled ?>"
                    style="background-color: #FFFFFF;  border: 1px solid #ADB5BD;"><a
                      href="<?= site_url('context/interested_party/timeline_log/' . $data['id_version'] . '/' . $data['type_version'] . '/' . $data['num_ver']); ?>"
  ...
  <script>
    function load_modal(check, data_encode) {
      ...
      //--set header--//
      var select_iss = <?php echo json_encode($data_interested_iss); ?>;
      $(".modal-header #title_modal").text("Interested Party");
      $(".modal-body #topic_title_select").text("Interested Party");
      ...
      if (check == '1') {
        ...
        $(".modal-body #url_route").val("context/interested_party/context_interested/store/" + element.id_version + "/" + element.status);
        ...
        $(".modal-body #url_route").val("context/interested_party/context_interested/edit/" + element.id_version + "/" + element.status);
  ...
  <script>
    var select_iss = document.getElementById("iss");

    function updateDescription() {

      var select_iss_data = <?php echo json_encode($data_interested_iss); ?>;

      let found = false;
      select_iss_data.forEach(element => {
        if (select_iss.value === element.id_select) {
          found = true;
    ...
    select_iss.addEventListener("change", function () {
      updateDescription();
    });
  </script>
  <script>
    ...
    function getTableData1() {
      ...
      if (countTable1 === 0) {
        ...
        $('#example1').DataTable({
          ...
          'serverSide': true,
          'ajax': {
            'url': "<?php echo site_url('context/interested_party/context_interested/find_data/'); ?>" + data_context.id_version,
            'type': 'GET',
            'dataSrc': 'data',
          },
          "responsive": true,
            ...
            'render': function (data, type, row, meta) {
              ...
              let dropdownHtml = `
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <a class="dropdown-item" onclick="load_modal(4, '${encodedRowData}')" data-toggle="modal"
                            data-target="#modal-default">Edit</a>
                        <a class="dropdown-item" href="#"
                            onclick="confirm_Alert('You want to copy data ${number_index} ?', 'context/interested_party/context_interested/copydata/${row.id_}/${number_index}/${data_context.id_version}/${data_context.status}')">Copy</a>
                        <a class="dropdown-item" href="#"
                            onclick="confirm_Alert('You want to delete data ${number_index} ?', 'context/interested_party/context_interested/delete/${row.id_}/${row.id_file}/${number_index}/${data_context.id_version}/${data_context.status}')">Delete</a>
                ...
                <div class="dropdown-divider"></div>
                  <div class="dropdown-submenu">
                      ...
                      <div class="dropdown-menu right-menu-table">
                          ...
                          <div class="dropdown-divider"></div>
                          <a class="dropdown-item" href="#"
                              onclick="confirm_Alert('You want to delete the data file ${number_index} ?', 'context/interested_party/context_interested/delete_file/${row.id_}/${row.id_file}/${number_index}/${data_context.id_version}/${data_context.status}')">Delete
                              File</a>
                      </div>
                  </div>`;
              }
          ...
          },
          {
            ...
            'class': 'text-center',
            'render': function (data, type, row, meta) {
              var data_select = <?php echo json_encode($data_interested_iss); ?>;
