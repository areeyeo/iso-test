                        ...
                        <?php if (!empty($consequence_level_is)): ?>
                            <nav style="border-color: #0000FF">
                                <div class="nav nav-tabs mt-3" id="nav-tab" role="tablist" >
                                    <?php foreach ($consequence_level_is as $item): ?>
                                        <?php if ($count_1 == 0):
                                            $count_1++ ?>
                                            <button class="nav-link active" style="color: #0000FF;"
                                                id="nav-<?php echo $item['id_consequence_level_is']; ?>-tab" data-toggle="tab"
                                                data-target="#nav-<?php echo $item['id_consequence_level_is']; ?>"
                                                type="button" role="tab"
                                                aria-controls="nav-<?php echo $item['id_consequence_level_is']; ?>"
                                                aria-selected="true">
                            ...
                            <div class="tab-content mt-3" id="nav-tabContent">
                                <?php foreach ($consequence_level_is as $key => $item) { ?>
                                    <?php if ($count_2 == 0):
                                        $count_2++; ?>
                                        <div class="tab-pane fade show active"
                                            id="nav-<?php echo $item['id_consequence_level_is']; ?>" role="tabpanel"
                                            aria-labelledby="nav-<?php echo $item['id_consequence_level_is']; ?>-tab"
                                            style="padding: 10px;">
                                        <?php else: ?>
                                            <div class="tab-pane fade" id="nav-<?php echo $item['id_consequence_level_is']; ?>"
                                                role="tabpanel"
                                                aria-labelledby="nav-<?php echo $item['id_consequence_level_is']; ?>-tab"
                                                style="padding: 10px;">
                                            <?php endif; ?>
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <button type="button" class="btn btn-outline-danger btn-sm"
                                                    ...
                                                    onclick="deleteConsequence(<?php echo $item['id_consequence_level_is']; ?>, '<?php echo $item['consequence_name']; ?>')">
...
<script>
    function load_modal(check, check_type, data_encode) {
        ...
        if (check == '1') {
            ...
            $(".modal-body #url_route").val('planning/risk_Criteria_IS_Consequence/create');
            ...
            var element = <?php echo json_encode($consequence_level_is); ?>;
            ...
            $(".modal-body #url_route").val('planning/risk_Criteria_IS_Consequence/edit/' + element[check_type].data_item[data_encode].id_consequence_level_item_is);
        }
    }
</script>
<script>
    function deleteConsequence(index, text) {
        var url_select = 'planning/risk_Criteria_IS_Consequence/delete/' + index;
...
<script>
    ...
    select_impact_level.addEventListener("change", function () {
        var url_select = 'planning/risk_Criteria_IS_Consequence/change_impact_level/' + select_impact_level.value;
