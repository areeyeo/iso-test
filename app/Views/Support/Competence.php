<title>Competence Version</title>
<!-- DataTables -->
    ...
    <script>
        function load_modal(check, check_type, data_encode) {
            ...
            if (check == '1') {
                ...
                //--show modal Support Competence --//
                modal_requirement.style.display = "none";
                modal_contextver.style.display = "none";
                modal_reject.style.display = "none";
                modal_crud_note.style.display = "none";
                modal_crud_support_competence.style.display = "block";
                $(".modal-body #role").val("Top Management");
                $(".modal-body #exampleInputFile").val("");
                    ...
                    return $.ajax({
                        ...
                        if (response.success) {
                            ...
                            setTimeout(() => {
                                if (response.reload) {
                                    if (response.newCopy) {
                                        window.location.href = '<?= site_url("support/competence/index/") ?>' + response.id_version + '/' + response.num_ver;
                                    } else {
                                        window.location.reload();
                                    }
                                }
        ...
        function getTableData1() {
            if (countTable1 === 0) {
                ...
                $('#example1').DataTable({
                    ...
                    'serverSide': true,
                    'ajax': {
                        'url': "<?php echo site_url('support/competence/getdata/'); ?>" + data_version.id_version,
                        'type': 'GET',
                        'dataSrc': 'data',
                    },
                    "responsive": true,
                            ...
                            let dropdownHtml = `
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <a class="dropdown-item" href="#"
                                                onclick="confirm_Alert('You want to copy data ${number_index} ?', 'support/competence/copydata/${data.id_competence}/${number_index}/${data_version.id_version}/${data_version.status}')">Copy</a>
                                            <a class="dropdown-item" href="#"
